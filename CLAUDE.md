...
**The whole match in track mode is a stream copy, not an encode** (`media/src/composite/copy.rs`). `ExportJob::render` picks the renderer and `composite::export::run` branches on it once, at the top; the `.part`, the chapters, the rename and the delete-on-failure stay in `run`, shared. The graph is `filesrc ! qtdemux ! queue ! h264parse` into a shared `concat` and `queue ! aacparse` into a second one, both into `mp4mux ! filesink` — no decoder, no encoder, no GL.
- **A `queue` after every demux pad is not optional:** without them the graph deadlocks on the first file, because `qtdemux`'s one streaming thread pushes video into an aggregator waiting on that same thread's audio.
- **`concat` numbers its sink pads in request order,** whatever name you ask for, so they are all requested up front in entry order — never in a `pad-added` handler, which runs on one thread per file and in no order at all. The copy iterates `plan.entries` (and `job.sources[entry.source_index]`), never `project.source_videos`: a source with no usable duration makes no entry.
...
- **The muxer's audio pad is requested by the first audio pad any file declares.** `qtdemux` adds every pad before it streams and `mp4mux` refuses a pad once it has started, so that ordering is what makes a source with no sound work rather than hang.
