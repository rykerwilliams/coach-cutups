...
**The whole match in track mode is a stream copy, not an encode** (`media/src/composite/copy.rs`). `ExportJob::render` picks the renderer and `composite::export::run` branches on it once, at the top; the `.part`, the chapters, the rename and the delete-on-failure stay in `run`, shared. **Rust owns the ordering, as the encode path's pump does:** one source at a time is opened as `filesrc ! qtdemux ! h264parse ! appsink` (and `! aacparse ! appsink`), run to EOS, and closed; its own demuxer thread carries each packet into `appsrc ! mp4mux ! filesink`. No decoder, no encoder, no GL, and **no `concat`** — two of those, one per track, switch source independently, so the muxer could wait for sound from source 2 while source 2's single `qtdemux` thread was blocked pushing picture into a queue the video `concat` had not reached: a deadlock, one run in three under load.
- **Only one thing in the copy ever waits, and its condition is the proof:** a push waits for room **only while every muxer pad already has a packet**. The muxer writes the earliest across its pads, so while all of them are fed it can always write, which drains a pad, which ends the wait — and a pad the muxer is waiting for is never the one held back. A per-source `live` flag ends the wait on any teardown, before the pipeline is taken to NULL and waits for that very thread.
- **The headers are read first, in a pass of their own** (`filesrc ! qtdemux ! fakesink`, left in `PAUSED`), because the gate must refuse before a byte is written and the muxing pipeline — and so the `.part` — is built only once every source has passed. A `fakesink`'s preroll is what stops a header pass reading further than a packet.
- **The copying `appsink`s are `async=false`.** A bin will not commit `PLAYING` while a sink's async state change is outstanding, and with two sinks on one demuxer thread and no queues the second one's never completes — the first blocks that thread in preroll first. In the header pass that same preroll is the bound that is wanted.
- **The packets are never re-timestamped.** Each rides a copy of `qtdemux`'s own segment with the *base* set to where its source starts in the output, so PTS, DTS and edit lists reach `mp4mux` as the file wrote them. **Clear that segment's `stop`:** `appsrc` adopts the segment it is handed, and `basesrc` ends the stream the moment a buffer passes it — measured as "everything after the first source is silently dropped". **One base advances both tracks**, which is what stops the ~5 ms-a-source A/V drift `concat`'s per-track `adjust-base` accumulated; the price is a join gap of one audio packet on the shorter track.
- The copy iterates `plan.entries` (and `job.sources[entry.source_index]`), never `project.source_videos`: a source with no usable duration makes no entry.
- **`trak-timescale` is pinned** — 90000 on video, the sample rate on audio — and **`reserved-max-duration` is set by the same formula the encoded path uses** (`export::reserved_duration`), because `chapters::splice` needs the `moov` first with a `free` after it. `reserved-duration-remaining`, read at EOS, is what says the margin is still ample on a longer match.
- **The caps gate is the only thing that refuses mismatched sources.** A 320×240 and a 640×480 file concatenate with **no error and no warning** into one file with one `stsd` describing most of its samples wrongly (measured). The gate reads `qtdemux`'s own negotiated caps in `pad-added` — H.264 and AAC absolutely, `codec_data` byte-identical to the first entry relatively — and every refusal names the file and says to choose **Scoreboard: burned in**. It waits for `no-more-pads` **with a bound**, like every other wait here.
- **The muxer's pads are requested from what the gate found**, before it starts: `mp4mux` refuses a pad once it has, so a project with no sound gets no audio track and one with sound gets it at the rate the gate read.
- **The scoreboard sidecar is `job.path.with_extension("srt")`** — `ExportJob::cues` written as `core::cues::cues_to_srt`, in `composite::export`'s `finish`, **after** the rename, so it inherits the run's name cleaning and its `" (2)"` de-duplication and is the matching basename a player auto-loads. A failure is logged and reported as `ExportDone::sidecar: None`; a good `.mp4` is never thrown away over a text file, and a cancel, which never reaches the rename, leaves the last good export's `.srt` alone.
- **A run that writes no sidecar removes a stale one, in every mode.** Writing and removing are one step: the file that belongs beside this output is this string, or nothing. Otherwise a coach exports with a scoreboard, deletes it or switches the picker back to burned in, exports again — and their player plays the old score over the new film.
...
- **"Default" is per target,** `core::plan::default_scoreboard_mode`: a track for the whole match, burned in for everything else. One rule, read by the sheet and the job builder alike.
- **The whole mapping is `carry_scoreboard` in `bus/export.rs`** — mode + target into the three job fields that carry the board, and nothing else. **Track mode blanks `job.scoreboard`** rather than carrying a mode flag into media: `None` is already media's one "don't draw the board", so there is no third state to keep consistent and `overlay.rs` never learns a picker exists. **`Render::Copy` and a cue list are the whole match's alone**: a clip or a reel on a separate track still re-encodes, for its drawings and its zoom, and gets no sidecar — it says what it is in its own text bar (spec T1). **With no `ScoreboardContext` there are no cues either** — nothing to derive them from.
