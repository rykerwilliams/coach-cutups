//! Export frame schedule: which source time and zoom land at each output frame.

use uuid::Uuid;

use video_coach_core::event::{CommentaryEvent, EventKind};
use video_coach_core::export::{frame_schedule, OUTPUT_FPS};
...
fn frame_count_covers_the_total() {
    ...
    assert_eq!(frame_schedule(&clip(0.0, 2.01, vec![]), DUR).len(), 61);
    assert!(frame_schedule(&clip(0.0, 0.0, vec![]), DUR).is_empty());
}
...
fn float_noise_in_the_total_adds_no_frame() {
    ...
    assert_eq!(frame_schedule(&clip(0.0, 8.3, vec![]), DUR).len(), 249);
    // The same total built from a sum of segments.
    let c = clip(0.0, 8.3, vec![pause(0.1, 0.1), play(0.2, 0.1)]);
    assert_eq!(frame_schedule(&c, DUR).len(), 249);
}

#[test]
fn play_maps_output_time_onto_the_source() {
    let s = frame_schedule(&clip(10.0, 1.0, vec![]), DUR);
...
fn playing_off_the_source_end_freezes_short_of_it() {
    ...
    let s = frame_schedule(&clip(9.0, 2.0, vec![]), 10.0);
    assert_eq!(s.len(), 60);
