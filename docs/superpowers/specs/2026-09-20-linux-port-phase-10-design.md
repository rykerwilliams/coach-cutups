...
The worker follows export exactly: a named `std::thread`, an `Arc<AtomicBool>` cancel, `on_message` called on the worker thread and forwarded into the bus's single `mpsc::Receiver<Input>`, and a `Drop` that cancels and joins.
